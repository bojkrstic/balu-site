export const contactPhone = '065 310 4333'

export const testimonials = [
  {
    quote:
      'Predivan ambijent, mnogo topline i strpljenja. Osećali smo se sigurno od prvog dolaska, a beba je baš uživala.',
    author: 'Roditelji iz Pirota',
  },
  {
    quote:
      'Slana soba i ceo prostor ostavljaju utisak mira i čistoće. Zakazivanje je bilo brzo, a pristup vrlo prijatan i profesionalan.',
    author: 'Mama devojčice, Pirot',
  },
  {
    quote:
      'Najviše nam znači što je sve prilagođeno bebama i što postoji osećaj pažnje prema svakom detalju tokom dolaska.',
    author: 'Porodica iz okoline Pirota',
  },
]

export const servicePages = {
  'slana-soba-pirot': {
    path: '/slana-soba-pirot/',
    eyebrow: 'Slana Soba Pirot',
    title: 'Slana soba u Pirotu za decu i odrasle',
    intro:
      'Balu Baby Spa u Pirotu nudi prijatan i miran prostor za boravak u slanoj sobi, namenjen deci, roditeljima i odraslima koji žele prirodno okruženje za opuštanje i lakše disanje.',
    description:
      'Slana soba u Pirotu za decu i odrasle u okviru Balu Baby Spa centra. Brzo zakazivanje termina putem WhatsApp-a i Vibera.',
    keywords:
      'slana soba Pirot, slana soba za decu Pirot, slana soba za odrasle Pirot, balu pirot',
    benefits: [
      'prijatan ambijent za decu i odrasle',
      'prostor usmeren na lakše disanje i opuštanje',
      'mogućnost porodičnog boravka u mirnom okruženju',
      'brzo zakazivanje termina putem WhatsApp-a i Vibera',
    ],
    goodFor: [
      'decu koja vole mirnu senzornu igru i istraživanje',
      'roditelje koji žele prijatan porodični ambijent',
      'odrasle koji traže lokalnu slanu sobu u Pirotu',
    ],
    faq: [
      {
        question: 'Gde se nalazi slana soba u Pirotu?',
        answer: 'Balu Baby Spa nalazi se na adresi Danila Kiša br. 2, Pirot.',
      },
      {
        question: 'Da li je slana soba namenjena i odraslima?',
        answer: 'Da, prostor je prilagođen i deci i odraslima, uz prijatan i miran ambijent.',
      },
      {
        question: 'Kako mogu da zakažem termin?',
        answer: 'Termin možete zakazati putem WhatsApp-a ili Vibera na broj 065 310 4333.',
      },
    ],
  },
  'hidroterapija-za-bebe-pirot': {
    path: '/hidroterapija-za-bebe-pirot/',
    eyebrow: 'Hidroterapija Za Bebe Pirot',
    title: 'Hidroterapija za bebe u Pirotu',
    intro:
      'U Balu Baby Spa centru u Pirotu hidroterapija za bebe osmišljena je kao nežno iskustvo kroz pokret u vodi, u toplom i sigurnom ambijentu prilagođenom najmlađima.',
    description:
      'Hidroterapija za bebe u Pirotu u okviru Balu Baby Spa centra. Nežan pristup, topao ambijent i brzo zakazivanje termina.',
    keywords:
      'hidroterapija za bebe Pirot, baby spa Pirot, balu baby spa pirot, vezbice za bebe pirot',
    benefits: [
      'podrška koordinaciji i prirodnom pokretu',
      'opušten ambijent prilagođen bebama',
      'iskustvo koje roditeljima donosi osećaj sigurnosti i poverenja',
      'termini po zakazivanju sa jasnom pripremom za dolazak',
    ],
    goodFor: [
      'bebe u ranom razvojnom periodu',
      'roditelje koji žele nežan i siguran pristup',
      'porodice koje traže baby spa u Pirotu',
    ],
    faq: [
      {
        question: 'Šta je hidroterapija za bebe?',
        answer: 'To je nežan boravak i pokret u vodi u ambijentu prilagođenom bebama i njihovom uzrastu.',
      },
      {
        question: 'Da li se termin zakazuje unapred?',
        answer: 'Da, termini se zakazuju unapred kako bi dolazak bio organizovan i prilagođen bebi.',
      },
      {
        question: 'Gde se nalazi Balu Baby Spa u Pirotu?',
        answer: 'Na adresi Danila Kiša br. 2, Pirot.',
      },
    ],
  },
  'masaza-beba-pirot': {
    path: '/masaza-beba-pirot/',
    eyebrow: 'Masaza Beba Pirot',
    title: 'Masaža beba u Pirotu',
    intro:
      'Balu Baby Spa u Pirotu nudi nežnu masažu beba i razvojne aktivnosti u prostoru koji je osmišljen da pruži toplinu, smirenost i podršku prirodnom razvoju.',
    description:
      'Masaža beba u Pirotu u okviru Balu Baby Spa centra. Nežan dodir, miran ambijent i jednostavno zakazivanje termina.',
    keywords:
      'masaža beba Pirot, masaza beba pirot, razvojne aktivnosti za bebe Pirot, balu pirot',
    benefits: [
      'nežan pristup kroz dodir i opuštanje',
      'ambijent prilagođen bebama i roditeljima',
      'razvojne aktivnosti kao podrška ranom periodu',
      'jednostavno zakazivanje termina i dolaska',
    ],
    goodFor: [
      'roditelje koji traže masažu beba u Pirotu',
      'bebe kojima prija smiren i topao prostor',
      'porodice koje žele individualniji pristup i više poverenja',
    ],
    faq: [
      {
        question: 'Da li je masaža beba namenjena samo novorođenčadi?',
        answer: 'Program se prilagođava uzrastu deteta i organizuje se po zakazivanju.',
      },
      {
        question: 'Kako izgleda dolazak na masažu beba?',
        answer: 'Dolazak je miran i organizovan, uz prethodne smernice za roditelje i pripremu bebe.',
      },
      {
        question: 'Kako mogu da zakažem termin u Pirotu?',
        answer: 'Najbrže putem WhatsApp-a ili Vibera na broj 065 310 4333.',
      },
    ],
  },
}
